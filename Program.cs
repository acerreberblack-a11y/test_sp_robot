...
namespace SpravkoBot_AsSapfir
{
    internal class Program
    {
            ...
            try
            {
                ...
                foreach (var file in files)
                {
                    ...
                    try
                    {
                        ...
                        string excelPath = @"C:\Users\RobinSapAC\Desktop\02-04-2025\SpravkoBot_AsSapfir\bin\Debug\KA\Реестр по всем БЕ.xlsx";
                        ...
                        Console.WriteLine($"Выполняю фильтрацию по параметрам из заявки: БЕ:{requestManager.RequestBE}, ИНН:{requestManager.INN}, КПП:{requestManager.KPP}");
                        ...
                        var filteredRows = File.ReadLines(csvPath)
                            .Skip(1)
                            ...
                            .Where(columns => columns.Length > 10 &&
                                ...
                                searchNames.Contains(columns[1]) &&
                                searchInn.Contains(columns[4]) &&
                                searchKpp.Contains(columns[5]))
                            .Select(columns => new
                            {
                                Number = columns.ElementAtOrDefault(0) ?? "",
                        ...
                        {
                            log.Error("При выборке по ИНН и КПП нашлось несколько результатов. Просьба проверить корректность данных.");
                            foreach (var row in filteredRows)
                            {
                                log.Error(string.Join(" | ", row));
                            }
                            throw new Exception("При выборке по ИНН и КПП нашлось несколько результатов. Просьба проверить корректность данных.");
                        }
                        ...
                        if (!string.IsNullOrWhiteSpace(personnelNumber))
                        {
                            string[] signatories = personnelNumber.Split(new char[] { ';' }, StringSplitOptions.RemoveEmptyEntries);

                            signatory1 = signatories[0].Trim();
                            signatory2 = signatories.Length > 1 ? signatories[1].Trim() : signatory1;

                            Console.WriteLine($"Выбраны подписанты: Подписант 1 — {signatory1}, Подписант 2 — {signatory2}");
                        }
                        else
                        {
                        ...
                        try
                        {
                            ...
                            string be = GetValueByName(requestManager.RequestBE)
                                ?? throw new Exception("Не смог сопоставить БЕ с заявки из списка БЕ в конфиге, так как она отсутствует");
                            ...
                            sapfir.SetText("wnd[1]/usr/tabsTAB_STRIP/tabpSIVA/ssubSCREEN_HEADER:SAPLALDB:3010/tblSAPLALDBSINGLE/ctxtRSCSEL_255-SLOW_I[1,0]", "*");
                            // Переключение на вкладку NOSV
                            sapfir.SelectTab("wnd[1]/usr/tabsTAB_STRIP/tabpNOSV");
                            Thread.Sleep(1000);
                            // Ввод значений для счетов 62
                            string[] accounts62 = new string[]
                            {
                                "6201010101", "6201010201", "6201010301", "6201010401",
                                "6201010501", "6201010601", "6201020101", "6201030101"
                            };

                            for (int i = 0; i < accounts62.Length; i++)
                            {
                                sapfir.SetText($"wnd[1]/usr/tabsTAB_STRIP/tabpNOSV/ssubSCREEN_HEADER:SAPLALDB:3030/tblSAPLALDBSINGLE_E/ctxtRSCSEL_255-SLOW_E[1,{i}]", accounts62[i]);
                            }

                            sapfir.SetVerticalScrollPosition("wnd[1]/usr/tabsTAB_STRIP/tabpSIVA/ssubSCREEN_HEADER:SAPLALDB:3030/tblSAPLALDBSINGLE_E", 7);
                            Thread.Sleep(1000);

                            string[] accounts62_2 = new string[]
                            {
                                "6201030201", "6201030301", "6201030401", "6201030501",
                                "6201030601", "6201030701", "6201040201"
                            };

                            for (int i = 1; i <= accounts62_2.Length; i++)
                            {
                                sapfir.SetText($"wnd[1]/usr/tabsTAB_STRIP/tabpNOSV/ssubSCREEN_HEADER:SAPLALDB:3030/tblSAPLALDBSINGLE_E/ctxtRSCSEL_255-SLOW_E[1,{i}]", accounts62_2[i - 1]);
                            }
                            ...
                            string[] accounts62_3 = new string[]
                            {
                                "6201040301", "6201110101", "6201130101", "6202010101",
                                "6202010201", "6202010301", "6202010401"
                            };

                            for (int i = 1; i <= accounts62_3.Length; i++)
                            {
                                sapfir.SetText($"wnd[1]/usr/tabsTAB_STRIP/tabpNOSV/ssubSCREEN_HEADER:SAPLALDB:3030/tblSAPLALDBSINGLE_E/ctxtRSCSEL_255-SLOW_E[1,{i}]", accounts62_3[i - 1]);
                            }
                            ...
                            string[] accounts62_4 = new string[]
                            {
                                "6202010501", "6202010601", "6202020101", "6202030101",
                                "6202030201", "6202030301", "6202030401"
                            };

                            for (int i = 1; i <= accounts62_4.Length; i++)
                            {
                                sapfir.SetText($"wnd[1]/usr/tabsTAB_STRIP/tabpNOSV/ssubSCREEN_HEADER:SAPLALDB:3030/tblSAPLALDBSINGLE_E/ctxtRSCSEL_255-SLOW_E[1,{i}]", accounts62_4[i - 1]);
                            }
                            ...
                            string[] accounts62_5 = new string[]
                            {
                                "6202030501", "6202030601", "6202030701", "6202040201",
                                "6202040301", "6202110101", "6202120101"
                            };

                            for (int i = 1; i <= accounts62_5.Length; i++)
                            {
                                sapfir.SetText($"wnd[1]/usr/tabsTAB_STRIP/tabpNOSV/ssubSCREEN_HEADER:SAPLALDB:3030/tblSAPLALDBSINGLE_E/ctxtRSCSEL_255-SLOW_E[1,{i}]", accounts62_5[i - 1]);
                            }
                            ...
                            string[] accounts76_1 = new string[]
                            {
                                "6202130101", "7602020101", "7602040101", "7611010101",
                                "7615020101", "7602010102", "7602020102"
                            };

                            for (int i = 1; i <= accounts76_1.Length; i++)
                            {
                                sapfir.SetText($"wnd[1]/usr/tabsTAB_STRIP/tabpNOSV/ssubSCREEN_HEADER:SAPLALDB:3030/tblSAPLALDBSINGLE_E/ctxtRSCSEL_255-SLOW_E[1,{i}]", accounts76_1[i - 1]);
                            }

                            // Прокрутка таблицы до позиции 42
                            sapfir.SetVerticalScrollPosition("wnd[1]/usr/tabsTAB_STRIP/tabpSIVA/ssubSCREEN_HEADER:SAPLALDB:3030/tblSAPLALDBSINGLE_E", 42);
                            Thread.Sleep(1000);

                            string[] accounts76_2 = new string[]
                            {
                                "7602030102", "7602010101", "7602040102", "7615020102", "760903*"
                            };

                            for (int i = 1; i <= accounts76_2.Length; i++)
                            {
                                sapfir.SetText($"wnd[1]/usr/tabsTAB_STRIP/tabpNOSV/ssubSCREEN_HEADER:SAPLALDB:3030/tblSAPLALDBSINGLE_E/ctxtRSCSEL_255-SLOW_E[1,{i}]", accounts76_2[i - 1]);
                            }
                            ...
                            if (serviceType == "По одному договору")
                            {
                                ...
                                string[] agreementNumbers = requestManager.AgreementNumber?.Split(new[] { ',' }, StringSplitOptions.RemoveEmptyEntries)
                                                        .Select(n => n.Trim())
                                ...
                                foreach (var number in agreementNumbers)
                                {
                                    if (!Regex.IsMatch(number.ToString(), @"^[\d/\-\.]+$"))
                                        throw new ArgumentException($"Номер договора '{number}' должен содержать только цифры и символы /, -, .");
                                }
                            ...
                            if (windowActive != "Акт сверки расчетов с контрагентами: ALV отчет" && !string.IsNullOrWhiteSpace(statusBarValue))
                            {
                                log.Error($"Данные по контрагенту не найдены или допущены ошибки в заполнении. Статус окна: {statusBarValue}. Перехожу к следующей заявке.");
                                throw new Exception();
                            }
                            ...
                            try
                            {
                                ...
                                if (button != null)
                                {
                                    sapfir.PressButton("wnd[1]/usr/btnBUTTON_1"); // Нажатие кнопки "Сформировать еще раз", если она есть
                            ...
                            sapfir.SetText("wnd[2]/usr/tabsG_SELONETABSTRIP/tabpTAB001/ssubSUBSCR_PRESEL:SAPLSDH4:0220/sub:SAPLSDH4:0220/txtG_SELFLD_TAB-LOW[0,24]", signatory1);
                            sapfir.SetText("wnd[2]/usr/tabsG_SELONETABSTRIP/tabpTAB001/ssubSUBSCR_PRESEL:SAPLSDH4:0220/sub:SAPLSDH4:0220/ctxtG_SELFLD_TAB-LOW[1,24]", "");
                            sapfir.SetText("wnd[2]/usr/tabsG_SELONETABSTRIP/tabpTAB001/ssubSUBSCR_PRESEL:SAPLSDH4:0220/sub:SAPLSDH4:0220/ctxtG_SELFLD_TAB-LOW[5,24]", "");
                            sapfir.SetText("wnd[2]/usr/tabsG_SELONETABSTRIP/tabpTAB001/ssubSUBSCR_PRESEL:SAPLSDH4:0220/sub:SAPLSDH4:0220/ctxtG_SELFLD_TAB-LOW[6,24]", "");
                            sapfir.ShowContextText("wnd[2]/usr/tabsG_SELONETABSTRIP/tabpTAB001/ssubSUBSCR_PRESEL:SAPLSDH4:0220/sub:SAPLSDH4:0220/ctxtG_SELFLD_TAB-LOW[6,24]"); // Установка фокуса
                            Thread.Sleep(1000); // Пауза 1 секунда
                            ...
                            sapfir.SetText("wnd[2]/usr/tabsG_SELONETABSTRIP/tabpTAB001/ssubSUBSCR_PRESEL:SAPLSDH4:0220/sub:SAPLSDH4:0220/txtG_SELFLD_TAB-LOW[0,24]", signatory2);
                            sapfir.SetText("wnd[2]/usr/tabsG_SELONETABSTRIP/tabpTAB001/ssubSUBSCR_PRESEL:SAPLSDH4:0220/sub:SAPLSDH4:0220/ctxtG_SELFLD_TAB-LOW[1,24]", "");
                            sapfir.SetText("wnd[2]/usr/tabsG_SELONETABSTRIP/tabpTAB001/ssubSUBSCR_PRESEL:SAPLSDH4:0220/sub:SAPLSDH4:0220/ctxtG_SELFLD_TAB-LOW[5,24]", "");
                            sapfir.SetText("wnd[2]/usr/tabsG_SELONETABSTRIP/tabpTAB001/ssubSUBSCR_PRESEL:SAPLSDH4:0220/sub:SAPLSDH4:0220/ctxtG_SELFLD_TAB-LOW[6,24]", "");
                            sapfir.ShowContextText("wnd[2]/usr/tabsG_SELONETABSTRIP/tabpTAB001/ssubSUBSCR_PRESEL:SAPLSDH4:0220/sub:SAPLSDH4:0220/ctxtG_SELFLD_TAB-LOW[6,24]"); // Установка фокуса
                            ...
                            if (grid != null)
                            {
                                ...
                                sapfir.SetText("wnd[2]/usr/tabsTAB_STRIP/tabpNOSV/ssubSCREEN_HEADER:SAPLALDB:3030/tblSAPLALDBSINGLE_E/ctxtRSCSEL_255-SLOW_E[1,0]", "@5b@");
                                ...
                                sapfir.RadioButton("wnd[1]/usr/subSUBSCREEN_STEPLOOP:SAPLSPO5:0150/sub:SAPLSPO5:0150/radSPOPLI-SELFLAG[1,0]", true);
                                ...
                                //string fileNameToSave = $"AktSverki_{requestManager.RequestBE}_{DateTime.Now:yyyyMMdd_HHmmss}.pdf"; // Пример имени файла
                                ...
                                using (var automation = new UIA3Automation())
                                {
                                    ...
                                    for (int attempt = 1; attempt <= MaxAttempts && !mainWindowFound; attempt++)
                                    {
                                        try
                                        {
                                            ...
                                            // Ожидание появления главного окна SAP "Акт сверки расчетов с контрагентами: ALV отчет"
                                            var mainWindow = desktop.FindFirstChild(cf =>
                                            ...
                                            while (mainWindow == null && (DateTime.Now - startTime).TotalSeconds < TimeoutSeconds)
                                            {
                             
                                                Thread.Sleep(1000); // Проверяем каждую секунду
                                                mainWindow = desktop.FindFirstChild(cf =>
                                                    cf.ByName("Акт сверки расчетов с контрагентами: ALV отчет")
                                                      .Or(cf.ByControlType(FlaUI.Core.Definitions.ControlType.Window)))
                                                    ?.AsWindow();
                                            }
                                            ...
                                            if (mainWindow == null)
                                            {
                                                log.Info($"Попытка {attempt}: Окно 'Акт сверки расчетов с контрагентами: ALV отчет' не найдено");
                                            ...
                                            var childWindow = mainWindow.FindFirstChild(cf =>
                                                cf.ByName("Акт сверки расчетов с контрагентами: ALV отчет")
                                                  .Or(cf.ByControlType(FlaUI.Core.Definitions.ControlType.Window)))
                                                ?.AsWindow();

                                            startTime = DateTime.Now;
                                            while (childWindow == null && (DateTime.Now - startTime).TotalSeconds < TimeoutSeconds)
                                            {
                                                Thread.Sleep(1000); // Проверяем каждую секунду
                                                childWindow = mainWindow.FindFirstChild(cf =>
                                                    cf.ByName("Акт сверки расчетов с контрагентами: ALV отчет")
                                                      .Or(cf.ByControlType(FlaUI.Core.Definitions.ControlType.Window)))
                                                    ?.AsWindow();
                                            }
                                            ...
                                            if (childWindow == null)
                                            {
                                                log.Info($"Попытка {attempt}: Дочернее окно 'Акт сверки расчетов с контрагентами: ALV отчет' не найдено");
                                                if (attempt == MaxAttempts)
                                                {
                                            ...
                                            log.Info("Дочернее окно 'Акт сверки расчетов с контрагентами: ALV отчет' найдено");
                                        ...
                                        {
                                            log.Error($"Попытка {attempt}: Ошибка при поиске окна SAP или отправке Enter - {ex.Message}");
                                            if (attempt == MaxAttempts)
                                            {
                                                throw new Exception("Не удалось обработать главное окно SAP после всех попыток.", ex);
                                            }
                                    ...
                                    for (int attempt = 1; attempt <= MaxAttempts; attempt++)
                                    {
                                        try
                                        {
                                            ...
                                            var browseWindow = desktop.FindFirstChild(cf =>
                                                cf.ByName("Browse for Files or Folders")
                                                  .Or(cf.ByControlType(FlaUI.Core.Definitions.ControlType.Window)))
                                                ?.AsWindow();

                                            DateTime startTime = DateTime.Now;
                                            while (browseWindow == null && (DateTime.Now - startTime).TotalSeconds < TimeoutSeconds)
                                            {
                                                Thread.Sleep(1000); // Проверяем каждую секунду
                                                browseWindow = desktop.FindFirstChild(cf =>
                                                    cf.ByName("Browse for Files or Folders")
                                                      .Or(cf.ByControlType(FlaUI.Core.Definitions.ControlType.Window)))
                                                    ?.AsWindow();
                                            }
                                            ...
                                            var folderEdit = browseWindow.FindFirstDescendant(cf =>
                                                cf.ByControlType(FlaUI.Core.Definitions.ControlType.Edit)
                                                  .And(cf.ByName("Folder:")))?.AsTextBox();
                                            ...
                                            // Ищем кнопку "OK"
                                            var okButton = browseWindow.FindFirstDescendant(cf =>
                                                cf.ByName("OK")
                                                  .And(cf.ByControlType(FlaUI.Core.Definitions.ControlType.Button)))?.AsButton();
        ...
        static string GetValueByName(string name)
        {
            ...
            {
                {"Бурейская ГЭС", "1030"}, {"Исполнительный аппарат", "1010"}, {"Волжская ГЭС", "1050"},
                {"Воткинская ГЭС", "1060"}, {"Дагестанский филиал", "1080"}, {"Жигулевская ГЭС", "1090"},
                {"Загорская ГАЭС", "1100"}, {"Зейская ГЭС", "1140"}, {"Кабардино-Балкарский филиал", "1170"},
                {"Камская ГЭС", "1180"}, {"Каскад Верхневолжских ГЭС", "1200"}, {"Каскад Кубанских ГЭС", "1210"},
                {"Нижегородская ГЭС", "1240"}, {"Новосибирская ГЭС", "1260"}, {"Саратовская ГЭС", "1300"},
                {"Северо-Осетинский филиал", "1320"}, {"Чебоксарская ГЭС", "1350"}, {"КорУнГ", "1410"},
                {"Хабаровский филиал", "1510"}, {"Приморский филиал", "1520"}, {"Якутский филиал", "1530"},
                {"Карачаево-Черкесский филиал", "1190"}, {"Саяно-Шушенская ГЭС (СШГЭС им. П.С. Непорожнего)", "1310"}
            };
