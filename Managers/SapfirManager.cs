...
namespace SpravkoBot_AsSapfir
{
    public class SapfirManager
    {
        ...
        {
            _pathSAPLoginApp = pathSAPLoginApp ?? throw new ArgumentNullException(nameof(pathSAPLoginApp), "Путь к SAP Logon не может быть null.");
        }
...
/*                // Если процесс уже запущен, просто выходим
                ...
                try
                {
                    ...
                    while (mainWindow == null && (DateTime.Now - startTime).TotalSeconds < 20)
                    {
                        mainWindow = desktop.FindFirstChild(cf =>
                            cf.ByName("SAP Logon 750")
                              .And(cf.ByControlType(FlaUI.Core.Definitions.ControlType.Window)))?.AsWindow();
        ...
        public GuiSession GetSapSession(string sapSystemName)
        {
            ...
            object engine = sapGuiRot.GetType().InvokeMember("GetScriptingEngine",
                System.Reflection.BindingFlags.InvokeMethod, null, sapGuiRot, null);
            if (engine == null)
                throw new Exception("Не удалось получить Scripting Engine.");

            SapApp = engine as GuiApplication ?? throw new Exception("Не удалось привести Scripting Engine к GuiApplication.");
            SapConnection = SapApp.OpenConnection(sapSystemName, false, true); // false - не асинхронно, true - ждать завершения
            SapSession = SapConnection.Children.ElementAt(0) as GuiSession ?? throw new Exception("Не удалось получить сессию.");
        ...
        {
            if (session == null) throw new ArgumentNullException(nameof(session), "Сессия не может быть null.");
            if (string.IsNullOrEmpty(user)) throw new ArgumentNullException(nameof(user), "Имя пользователя не может быть пустым.");
            if (string.IsNullOrEmpty(password)) throw new ArgumentNullException(nameof(password), "Пароль не может быть пустым.");

            Thread.Sleep(1000); // Ожидание для стабилизации интерфейса
            GuiMainWindow mainWindow = session.FindById("wnd[0]") as GuiMainWindow ?? throw new Exception("Главное окно wnd[0] не найдено.");
            GuiTextField userField = session.FindById("wnd[0]/usr/txtRSYST-BNAME") as GuiTextField ?? throw new Exception("Поле логина не найдено.");
            GuiTextField pwdField = session.FindById("wnd[0]/usr/pwdRSYST-BCODE") as GuiTextField ?? throw new Exception("Поле пароля не найдено.");
        ...
        {
            GuiToolbarControl toolbar = SapSession.FindById(id) as GuiToolbarControl ?? throw new Exception($"Кнопка панели инструментов с ID {id} не найдена.");
            toolbar.SetFocus();
        ...
        {
            GuiFrameWindow window = SapSession.FindById(id) as GuiFrameWindow ?? throw new Exception($"Окно с ID {id} не найдено.");
            window.SetFocus();
        ...
        {
            GuiComponent component = SapSession.FindById(id) as GuiComponent ?? throw new Exception($"Компонент с ID {id} не найден.");
        ...
        {
            GuiGridView grid = SapSession.FindById(id) as GuiGridView ?? throw new Exception($"GridView с ID {id} не найден.");
            grid.ContextMenu();
        ...
        {
            GuiCheckBox checkbox = SapSession.FindById(id) as GuiCheckBox ?? throw new Exception($"Чекбокс с ID {id} не найден.");
            checkbox.SetFocus();
        ...
        {
            GuiRadioButton radio = SapSession.FindById(id) as GuiRadioButton ?? throw new Exception($"Радиокнопка с ID {id} не найдена.");
            radio.SetFocus();
        ...
        {
            GuiCTextField textField = SapSession.FindById(id) as GuiCTextField ?? throw new Exception($"Поле с ID {id} не найдено или не является GuiCTextField.");
            textField.SetFocus();
        ...
        {
            GuiComboBox combo = SapSession.FindById(id) as GuiComboBox ?? throw new Exception($"Комбобокс с ID {id} не найден.");
            combo.SetFocus();
        ...
        {
            GuiTableControl table = SapSession.FindById(tableId) as GuiTableControl ?? throw new Exception($"Таблица с ID {tableId} не найдена.");
        ...
        {
            if (string.IsNullOrEmpty(windowTitle)) throw new ArgumentNullException(nameof(windowTitle), "Заголовок окна не может быть пустым.");
